...
def part1(filename):
	print("Part 1: {}".format([all(n - p in (1, 2, 3) for p, n in itertools.pairwise(report)) or all(n - p in (-1, -2, -3) for p, n in itertools.pairwise(report)) for report in (list(map(int, re.findall(r"(\d+)", line))) for line in io.open(filename, mode = 'r'))].count(True)))
